...
const getReward = async (wallet, liquidityMakerAddress) => {
    try {
        const { signer } = await helpers.web3.authentication.getSigner(wallet,wallet.chainId)
        const liquidityMaker = await new ethers.Contract(liquidityMakerAddress, LiquidityMaker.abi, signer);
...
const compoundLPReward = async (wallet, liquidityMakerAddress) => {
    try {
        const { signer } = await helpers.web3.authentication.getSigner(wallet,wallet.chainId)
        const liquidityMaker = await new ethers.Contract(liquidityMakerAddress, LiquidityMaker.abi, signer);
...
const addReward = async (wallet, liquidityMakerAddress, amount) => {
    try {
        const { signer } = await helpers.web3.authentication.getSigner(wallet,wallet.chainId)
        const liquidityMaker = await new ethers.Contract(liquidityMakerAddress, LiquidityMaker.abi, signer);
...
const addLiquidityReward = async (wallet, liquidityMakerAddress, amount) => {
    try {
        const { signer } = await helpers.web3.authentication.getSigner(wallet,wallet.chainId)
        const liquidityMaker = await new ethers.Contract(liquidityMakerAddress, LiquidityMaker.abi, signer);
...
const stake = async (wallet, liquidityMakerAddress, amount) => {
    try {
        const { signer } = await helpers.web3.authentication.getSigner(wallet,wallet.chainId)
        const liquidityMaker = await new ethers.Contract(liquidityMakerAddress, LiquidityMaker.abi, signer);
...
const setMaxTotalSupply = async (wallet, liquidityMakerAddress, amount) => {
    try {
        const { signer } = await helpers.web3.authentication.getSigner(wallet,wallet.chainId)
        const liquidityMaker = await new ethers.Contract(liquidityMakerAddress, LiquidityMaker.abi, signer);
...
const setLockingPeriod = async (wallet, liquidityMakerAddress, amount) => {
    try {
        const { signer } = await helpers.web3.authentication.getSigner(wallet,wallet.chainId)
        const liquidityMaker = await new ethers.Contract(liquidityMakerAddress, LiquidityMaker.abi, signer);
...
const fetchHoldersMapping = async (wallet, liquidityMakerAddress, address) => {
    try {
        ...
        console.log('Liquidity fetchHoldersMapping error', e);
        toast.error(e.reason);
        return 0;
    }
}

const getTVL = async (wallet, liquidityMakerAddress, baseTokenAddress, pairedTokenAddress, pairAddress, chainId = DEFAULT_CHAIN_ID) => {
    try {
        ...
        let baseBalance = await helpers.web3.token.balanceOf(wallet, baseTokenAddress, pairAddress,chainId)
        let pairedBalance = await helpers.web3.token.balanceOf(wallet, pairedTokenAddress, pairAddress,chainId)
        let totalSupply = await helpers.web3.token.fetchTotalSupply(wallet, pairAddress,chainId)
        ...
        console.log('getTvl Liquidity maker',e);
    }
}


const getCurrentValue = async (wallet, liquidityMakerAddress, address, baseTokenAddress, pairedTokenAddress,pairAddress) => {
    try {
        const { signer } = await helpers.web3.authentication.getSigner(wallet,wallet.chainId)
        const liquidityMaker = await new ethers.Contract(liquidityMakerAddress, LiquidityMaker.abi, signer);
        const data = await liquidityMaker.holdersMapping(address);
        ...
        let baseBalance = await helpers.web3.token.balanceOf(wallet, baseTokenAddress, pairAddress,wallet.chainId)
        let pairedBalance = await helpers.web3.token.balanceOf(wallet, pairedTokenAddress, pairAddress,wallet.chainId)
        let totalSupply = await helpers.web3.token.fetchTotalSupply(wallet, pairAddress,wallet.chainId)
        ...
        console.log('Liquidity getCurrentValue error', e);
        toast.error(e.reason);
        return 0;
    }
}
