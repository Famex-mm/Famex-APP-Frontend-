...
export default function FarmsCardItem({liquidityMaker}) {
    ...
    const verifyChain = (wallet, liqChainId) => {
        return wallet.chainId === Number(liqChainId) ? true : false
    }

    useEffect(() => {
        if (wallet.status === "connected" && liquidityMaker.address && verifyChain(wallet,liquidityMaker.network)) {
            const initWalletConnected = async () => {
                setRewardPerToken(
                    await helper.web3.liquidityMaker.rewardPerToken(wallet, liquidityMaker.address,liquidityMaker.network)
                );
                setLiquidityRewardPerToken(
                    await helper.web3.liquidityMaker.liquidityRewardPerToken(wallet, liquidityMaker.address,liquidityMaker.network)
                );
        ...
        if ((wallet.status === "disconnected" || !verifyChain(wallet,liquidityMaker.network)) && liquidityMaker.address) {
        ...
        { loading ? 
            ...
            <div className="flex flex-col p-6 bg-white rounded-2.5xl gap-5 divide-y">
                <div className="flex flex-col gap-4">
                    ...
                    {
                        wallet.status === "connected" && verifyChain(wallet,liquidityMaker.network) ? <>
                    ...
                    {
                        wallet.status === "connected" && verifyChain(wallet,liquidityMaker.network)? <ButtonFit name="Exit" handleClick={exitLiquidity} disabled={parseFloat(liquidityBalance) === 0.0}/>
                                :
                                ""
                    }
                    ...
                    {
                        wallet.status === "connected" && verifyChain(wallet,liquidityMaker.network) ?   <div className="flex justify-between">
                    ...
                    {
                        wallet.status === "connected" && verifyChain(wallet,liquidityMaker.network) ?  <div className="flex flex-col space-y-2.5 md-lg:flex-row md-lg:space-y-0 md-lg:items-center md-lg:justify-between text-base">
