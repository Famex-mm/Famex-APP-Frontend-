...
export default function MarketMakingDeployment({project, marketMakingPool, mainChainId}) {
    ...
    const loadWeb3 = async () => {
        ...
        if (marketMakingPool.paired_token === WETH_ADDRESS[wallet.chainId]) {
            ...
            setPairedTokenWalletBalance(helper.formatting.web3Format(await helper.token.balanceOf(wallet, marketMakingPool.paired_token, wallet.account, mainChainId)));
        }

        setBaseTokenWalletBalance(helper.formatting.web3Format(await helper.token.balanceOf(wallet, marketMakingPool.token, wallet.account,mainChainId)));
        const {
            balanceInBaseToken,
